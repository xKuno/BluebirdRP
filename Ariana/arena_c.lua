...
local areanaIPL = "xs_arena_interior"

function AreanaIPListing()
	return areanaIPL
end


RegisterNetEvent(  'loadareanaipl')
AddEventHandler(  'loadareanaipl', function(xPlayer)
	Citizen.CreateThread(function()
		-- New Arena : 2800.00, -3800.00, 100.00
		RequestIpl(areanaIPL)  -- Set Above
...
RegisterNetEvent(  'unloadareanaipl')
AddEventHandler(  'unloadareanaipl', function(xPlayer)
	Citizen.CreateThread(function()
		-- New Arena : 2800.00, -3800.00, 100.00
		...
		--RemoveIpl("xs_arena_interior_vip") -- This is the interior bar for VIP's
		--RemoveIpl("xs_int_placement_xs")
		...
		if (scene == "dystopian") then
			...
			print("[Arena by Titch]: enabling map: "..maps[scene][map])
			DisableInteriorProp(interiorID, maps[scene][map])
		end
		if (scene == "scifi") then
