...
AddEventHandler("changeWeather", function(newWeather,blackout,startup)
	changeWeather(newWeather,blackout,startup)
end)

function changeWeather(newWeather,blackout,startup)
	...
	if off == false then
		...
		missedNewWeather = newWeather
		missedblackout = blackout
		missedstartup = startup
		missedacycle = true
