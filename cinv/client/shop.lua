...
function OpenShopInv(shoptype)
    ...
    data = {text = text}
    inventory = {}
    ESX.TriggerServerCallback('d51e5311-bf5d-40ec-9e28-eb487a4ded52', function(shopInv)
        for i = 1, #shopInv, 1 do
            table.insert(inventory, shopInv[i])
            
        end
        TriggerEvent('cfae71d5-e044-4315-aa5c-a8a0a2fb2c6f', data, inventory)
    end, shoptype)
end

RegisterNetEvent('743edeb4-4e8b-41e2-9427-8e94232ee8d8')
AddEventHandler('743edeb4-4e8b-41e2-9427-8e94232ee8d8', function(type, shopinventory)
    text = "shop"
    data = {text = text}
    inventory = {}

    ESX.TriggerServerCallback('e406aef2-7f4d-4f43-87a6-2a76de161890', function(shopInv)
        for i = 1, #shopInv, 1 do
            table.insert(inventory, shopInv[i])
        end
        TriggerEvent('cfae71d5-e044-4315-aa5c-a8a0a2fb2c6f', data, inventory)
...
RegisterNetEvent('cfae71d5-e044-4315-aa5c-a8a0a2fb2c6f')
AddEventHandler('cfae71d5-e044-4315-aa5c-a8a0a2fb2c6f', function(data, inventory)
...
RegisterNUICallback("TakeFromShop", function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        TriggerServerEvent('ef66db08-7ce2-4120-996f-7dd8bb43ec1c', GetPlayerServerId(PlayerId()), data.item.type, data.item.name, tonumber(data.number))
    end
...
RegisterNetEvent('32bd3b3f-4dee-4726-ae70-25e9ed806ce4')
AddEventHandler('32bd3b3f-4dee-4726-ae70-25e9ed806ce4', function(hash, amount)
...
function IsInRegularShopZone(coords)
    ...
    for i = 1, #RegularShop, 1 do
        if #(coords - vector3(RegularShop[i].x, RegularShop[i].y, RegularShop[i].z)) < 2.0 then
            return true
        end
    end
    return false
...
function IsInIlegalShopZone(coords)
    ...
    for i = 1, #IlegalShop, 1 do
        if #(coords - vector3(IlegalShop[i].x, IlegalShop[i].y, IlegalShop[i].z)) < 2.0 then
            return true
        end
    end
    return false
...
function IsInRobsLiquorZone(coords)
    ...
    for i = 1, #RobsLiquor, 1 do
        if #(coords - vector3(RobsLiquor[i].x, RobsLiquor[i].y, RobsLiquor[i].z)) < 2.0 then
            return true
        end
    end
    return false
...
function IsInYouToolZone(coords)
    ...
    for i = 1, #YouTool, 1 do
        if #(coords- vector3(YouTool[i].x, YouTool[i].y, YouTool[i].z)) < 2.0 then
            return true
        end
    end
    return false
...
function IsInPrisonShopZone(coords)
    ...
    for i = 1, #PrisonShop, 1 do
        if #(coords - vector3(PrisonShop[i].x, PrisonShop[i].y, PrisonShop[i].z)) < 2.0 then
            return true
        end
    end
    return false
...
function IsInWeaponShopZone(coords)
    ...
    for i = 1, #WeaponShop, 1 do
        if #(coords - vector3(WeaponShop[i].x, WeaponShop[i].y, WeaponShop[i].z)) < 2.0 then
            return true
        end
    end
    return false
...
function IsInPoliceShopZone(coords)
    ...
    for i = 1, #PoliceShop, 1 do
        if #(coords - vector3(PoliceShop[i].x, PoliceShop[i].y, PoliceShop[i].z)) < 2.0 then
            return true
        end
    end
    return false
...
Citizen.CreateThread(function()
    ...
    CreateBlip(vector3(-755.79, 5596.07, 41.67), "Air Lift", 3.0, 4, 36)
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPedOnFoot(player) then
            for k, v in pairs(Config.Shops.RegularShop.Locations) do
                local distance = #(coords - vector3(Config.Shops.RegularShop.Locations[k].x, Config.Shops.RegularShop.Locations[k].y, Config.Shops.RegularShop.Locations[k].z))
                if distance < 10 then
            ...
            for k, v in pairs(Config.Shops.IlegalShop.Locations) do
                local distance = #(coords - vector3(Config.Shops.IlegalShop.Locations[k].x, Config.Shops.IlegalShop.Locations[k].y, Config.Shops.IlegalShop.Locations[k].z))
                if distance < 10 then
            ...
            for k, v in pairs(Config.Shops.RobsLiquor.Locations) do
                local distance = #(coords - vector3(Config.Shops.RobsLiquor.Locations[k].x, Config.Shops.RobsLiquor.Locations[k].y, Config.Shops.RobsLiquor.Locations[k].z))
                if distance < 10 then
            ...
            for k, v in pairs(Config.Shops.YouTool.Locations) do
                local distance = #(coords - vector3(Config.Shops.YouTool.Locations[k].x, Config.Shops.YouTool.Locations[k].y, Config.Shops.YouTool.Locations[k].z))
                if distance < 10 then
            ...
            for k, v in pairs(Config.Shops.PrisonShop.Locations) do
                local distance = #(coords - vector3(Config.Shops.PrisonShop.Locations[k].x, Config.Shops.PrisonShop.Locations[k].y, Config.Shops.PrisonShop.Locations[k].z))
                if distance < 10 then
            ...
            for k, v in pairs(Config.Shops.WeaponShop.Locations) do
                local distance = #(coords - vector3(Config.Shops.WeaponShop.Locations[k].x, Config.Shops.WeaponShop.Locations[k].y, Config.Shops.WeaponShop.Locations[k].z))
                if distance < 10 then
            ...
            for k, v in pairs(Config.Shops.PoliceShop.Locations) do
                local distance = #(coords - vector3(Config.Shops.PoliceShop.Locations[k].x, Config.Shops.PoliceShop.Locations[k].y, Config.Shops.PoliceShop.Locations[k].z))
                if distance < 10 then
