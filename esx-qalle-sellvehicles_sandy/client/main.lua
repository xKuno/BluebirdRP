...
RegisterNetEvent('ac9a7cf2-111a-46b2-99c4-f542326ab6ec')
AddEventHandler('ac9a7cf2-111a-46b2-99c4-f542326ab6ec', function()
	RemoveVehicles()
...
function LoadSellPlace()
	Citizen.CreateThread(function()
		...
		while true do
			...
			if dstCheck >= 45.0 and dstCheck <= 100.0 then
				sleepThread = 15
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		if price ~= nil then
			...
			table.insert(elements, { ["label"] = "<span style='color: red;'><b>*Sale Warning*:</b></span><span style='color: grey;'>Your car will be available for sale immediately.</span>", ["value"] = "  " })
			table.insert(elements, { ["label"] = "<span style='color: grey;'>You can withdraw your car at anytime from the lot providing it hasn't been already sold.</span>", ["value"] = "  " })
			table.insert(elements, { ["label"] = "<span style='color: grey;'>Vehicles not sold within 2 week period will be automatically returned to the owners garage in Zone 2.</span>", ["value"] = "  "})
			...
			table.insert(elements, { ["label"] = "<span style='color: grey;'>10% fee applies for successful sales. If you don't agree - do not sell your car.</span>", ["value"] = "  "			})
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			if (GetGameTimer() - GUI.Time) > 3000 then
				...
				ESX.TriggerServerCallback('8614f80a-3c60-43a4-8816-15de611d25a5', function(valid)
		...
		elseif action == "buy" then
			if GetVehiclePedIsIn(GetPlayerPed(-1),false) ~= 0 then
				if (GetGameTimer() - GUI.Time) > 3000 then
					GUI.Time  = GetGameTimer()
					ESX.TriggerServerCallback('7a19ad98-c848-40c3-9f4d-58e728501798', function(isPurchasable, totalMoney)
...
function SpawnVehicles()
	...
	ESX.TriggerServerCallback('64d8dd65-1f5f-4fbf-8f71-79e58f010666', function(vehicles)
		print(#vehicles)
