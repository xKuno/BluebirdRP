...
CreateThread(function()
    while true do
        if isWithinObject and oElement.fObject ~= 0 and not disabled or InUse == true then
            local ply = PlayerPedId()
            local objectCoords = oElement.fObjectCoords
            local distanceDiff = #(objectCoords - plyCoords)
            if (distanceDiff < 2.2 and not InUse) then
                if (oElement.fObjectIsBed == true) then
                    ...
                    if IsControlJustPressed(0, Config.objects.ButtonToLayOnBed) then
                        TriggerServerEvent('7eb75707-0bb0-472e-8f6c-1d90a42d31f4', oElement, oElement.fObjectCoords)
                    end
                else    
                    ...
                    if IsControlJustPressed(0, Config.objects.ButtonToSitOnChair) then
						--if HasEntityClearLosToEntityInFront(ply,oElement.fObject,17) then
							TriggerServerEvent('7eb75707-0bb0-472e-8f6c-1d90a42d31f4', oElement, oElement.fObjectCoords)
						--end
                    end
                end     
            ...
            if (InUse) then
                ...
                if IsControlJustPressed(0, Config.objects.ButtonToStandUp) or IsControlPressed(0, 74) then
					print('stand up')
                    InUse = false
                    TriggerServerEvent('6804dd72-9f09-443e-8e49-0054ec18ef20', oElement.fObjectCoords)
...
CreateThread(function()
    while true do
        plyCoords = GetEntityCoords(PlayerPedId())  --ESX.Game.MyCoords()
        Wait(1000)
    end
end)
...
CreateThread(function()
    ...
    while true do
        ...
        for _, element in pairs(Config.objects.locations) do
            local closestObject = GetClosestObjectOfType(plyCoords.x, plyCoords.y, plyCoords.z, 20.0, tonumber(element.object), 0, 0, 0)
            local coordsObject = GetEntityCoords(closestObject)
            local distanceDiff = #(coordsObject - plyCoords)
            if (distanceDiff < 25.0 and closestObject ~= 0) then
				table.insert(tempTable,element)
				
            end
			Wait(100)
        end
		nearObjects = tempTable
        Wait(3500)
    end
end)
...
CreateThread(function()
    ...
    while true do
        for _, element in pairs(nearObjects) do
            local closestObject = GetClosestObjectOfType(plyCoords.x, plyCoords.y, plyCoords.z, 3.0, element.object, 0, 0, 0)
                ...
                if (distanceDiff < 2.0) then
					if element.nochange ~= nil then
					 oElement = {
						...
						fObjectNoChange = element.nochange
...
RegisterNetEvent('d299c2d6-125e-4c2a-bac9-fd81afc7d834')
AddEventHandler('d299c2d6-125e-4c2a-bac9-fd81afc7d834', function(v, coords)
    ...
    if isBed == false then
        if Config.objects.SitAnimation.dict ~= nil then
			...
			if v.fObjectNoChange == nil then
				...
				local myhead = GetEntityHeading(ped)
				TaskStartScenarioAtPosition(ped, Config.objects.SitAnimation.anim, objectcoords.x + vertx, objectcoords.y + verty, objectcoords.z - vertz, myhead, 0, true, true)
			end
		end
    else
        if Anim == 'back' then
            if Config.objects.BedBackAnimation.dict ~= nil then
                SetEntityCoords(ped, objectcoords.x, objectcoords.y, objectcoords.z - vertz)
                SetEntityHeading(ped, GetEntityHeading(object) - 180.0)
        ...
        elseif Anim == 'stomach' then
            if Config.objects.BedStomachAnimation.dict ~= nil then
                ...
                Animation(dict, anim, ped)
			elseif  Config.objects.BedStomachAnimation.anim  ~= nil   then
				
				SetEntityCoords(ped, objectcoords.x, objectcoords.y, objectcoords.z + 0.50)
                SetEntityHeading(ped, GetEntityHeading(object) - 90.0)
 
                TaskStartScenarioAtPosition(ped, Config.objects.BedStomachAnimation.anim, objectcoords.x + vertx, objectcoords.y + verty, objectcoords.z + 1.5, GetEntityHeading(object) , 0, true, true)
            else
				
                TaskStartScenarioAtPosition(ped, Config.objects.BedStomachAnimation.anim, objectcoords.x + vertx, objectcoords.y + verty, objectcoords.z - vertz, GetEntityHeading(object) , 0, true, true)
            end
        elseif Anim == 'sit' then
            if Config.objects.BedSitAnimation.dict ~= nil then
			   ...
			   SetEntityCoords(ped, objectcoords.x, objectcoords.y, objectcoords.z + 0.50)
                ...
                Animation(dict, anim, ped)
				SetEntityCoords(ped, objectcoords.x, objectcoords.y, objectcoords.z + 0.50)
			elseif  Config.objects.BedSitAnimation.anim  ~= nil   then
			
				
				SetEntityCoords(ped, objectcoords.x, objectcoords.y, objectcoords.z + 0.50)
                SetEntityHeading(ped, GetEntityHeading(object) - 180.0)
 
                TaskStartScenarioAtPosition(ped, Config.objects.BedSitAnimation.anim, objectcoords.x + vertx, objectcoords.y + verty, objectcoords.z + 1.5, GetEntityHeading(object) + 180.0, 0, true, true)
            else
...
RegisterNetEvent('749862c0-d5c2-4904-a643-f69db2d94845')
AddEventHandler('749862c0-d5c2-4904-a643-f69db2d94845', function(value)
	if value == true then
		disabled = true
	else
		disabled = false
