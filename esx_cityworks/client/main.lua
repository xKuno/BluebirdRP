...
function StopNPCJob(cancel)
    ...
    if cancel then
	  ESX.ShowNotification(_U('cancel_mission'))
		...
		TriggerServerEvent('b03a621f-f7d4-4cfa-83fd-a8d2a74118df')
		StartNPCJob()
		Done = true
	end
end
...
AddEventHandler('093f76e5-d4ba-4b3d-88ae-f0055f02a42f', function(zone)
...
AddEventHandler('96ba0d3d-b994-4f3e-9244-f3e1e8baa8d1', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('569a8033-0264-436c-b214-f6b6de6c129c')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('093f76e5-d4ba-4b3d-88ae-f0055f02a42f', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('96ba0d3d-b994-4f3e-9244-f3e1e8baa8d1', LastZone)
			end
		else
			Wait(3000)
		end
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
				...
				if PlayerData.job.name == Config.nameJob then
					...
					if CurrentAction == 'vente' then
						TriggerServerEvent('1fce0157-de36-42a7-8559-cdd058d07052')
	...
	while true do
		...
		if PlayerData.job ~= nil then
			if PlayerData.job.name ~= Config.nameJob then
				...
				if IsControlJustReleased(1, Keys["F10"]) and PlayerData.job ~= nil then
