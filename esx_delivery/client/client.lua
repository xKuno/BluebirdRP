...
Citizen.CreateThread(function()
	while true do
		...
		if starteduber and not hasdelivery then
			cooldown = math.random(30000,65000)
			Citizen.Wait(cooldown) -- [Debug]
		--	print('Delivery Sent')  -- [Debug]
			TriggerEvent('75380609-4b35-4a79-a85c-f2b383c76722', 'pager', 0.4)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		if starteduber == true and hasdelivery == true then
...
function rewarduber()
	if ispedhomechance == 1 then
		...
		price = math.random(110,245)
		TriggerServerEvent('dd1b5cdc-a2cf-4ae0-aea4-d530033461bc', price)
		...
		if starteduber then
			...
			if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), DeliveryLocations[1]["x"], DeliveryLocations[1]["y"], DeliveryLocations[1]["z"], true ) < 1.2 and deliverydoor1 and hasdelivery then --and PlayerData.job ~= nil and PlayerData.job.name == 'mecano'
					...
					if(IsControlJustPressed(1, Keys["E"])) and not deliveryporch1 and not disablepressE then
						...
						Citizen.Wait(1000)
						TriggerServerEvent('bdf7ee62-47d6-45c0-b1d7-a575b267e734', 5.0, 'doorknock', 0.5)
						Citizen.Wait(2000)
						doorknocked1 = true
...
RegisterCommand('uber', function(source, args, rawCommand)
 ...
 if not starteduber and uerdeliveryamount ~= 10 then
	exports['mythic_notify']:SendAlert('success', 'Uber Eats Delivery: Active [Wait for Delivery]')
	 starteduber = true
	 hasdelivery = false
	-- print(starteduber)
	elseif starteduber then
	exports['mythic_notify']:SendAlert('error', 'Uber Eats Delivery: Deactivated')
	 starteduber = false
	 hasdelivery = true
	 -- print(starteduber)	
	 removeblips()
