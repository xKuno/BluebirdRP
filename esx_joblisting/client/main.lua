...
function ShowJobListingMenu(data)
	ESX.TriggerServerCallback('6f8db613-d603-4ad1-ad63-f5fc3e7f73ae', function(data)
		local elements = {}
		...
		'default', GetCurrentResourceName(), 'joblisting',
		{
			...
			TriggerServerEvent('537fde76-707c-4c12-94dd-96ed1da63882', data.current.value)
			
			
			ESX.ShowNotification(_U('new_job'))
			Citizen.CreateThread(function()
				Wait(2000)
				ESX.ShowNotification('~g~Congrats~w~ on the new job buddy.\nYou job will likely have markers now on your map.')
				Wait(2000)
				ESX.ShowNotification('~o~What If I cant find my job?~w~ Ask in roleplay.')
				Wait(1500)
				ESX.ShowNotification('~y~There are people who like to help, but may do so for a price.')
			end)
			menu.close()
...
AddEventHandler('bf7381c9-d5d1-4526-bea0-5d96d9a4bd52', function(zone)
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if closer then
			...
			if not isInJoblistingMarker and hasAlreadyEnteredMarker then
				...
				TriggerEvent('bf7381c9-d5d1-4526-bea0-5d96d9a4bd52')
			end
		else
			Wait(2000)
		end
