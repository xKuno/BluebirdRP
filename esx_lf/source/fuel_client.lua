...
function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		local newval = GetVehicleFuelLevel(vehicle) - Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (Config.Classes[GetVehicleClass(vehicle)] or 1.0) / 10
		local speed = GetEntitySpeed(vehicle) *3.6
		if speed > 220 then
			newval = newval - (speed/2450)
		end
		SetFuel(vehicle, newval)
		lastcachedval = newval
	end
end
...
AddEventHandler('0163347a-34b9-4cc6-a337-58fc26599cc7', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if not pumpObject then
			if GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100 >= 0 then
				...
				TriggerServerEvent('0c89ee80-d0a7-4271-9bce-01fcfb1ec136',Round(fuelToAdd+math.random(100,400),0))
				--SetPedAmmo(ped, 883325847, math.floor(GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100))
			else
				TriggerServerEvent('2111f846-8241-49b7-bd60-39b8b022110c',GetVehicleNumberPlateText(vehicle),currentFuel,'115')
				isFueling = false
			end
		else
			currentFuel = oldFuel + fuelToAdd
		...
		if currentCash >= currentCost then
			...
			isFueling = false
			TriggerServerEvent('2111f846-8241-49b7-bd60-39b8b022110c',GetVehicleNumberPlateText(vehicle),currentFuel,'134')
		end
	end

	if pumpObject then
		TriggerServerEvent('2111f846-8241-49b7-bd60-39b8b022110c',GetVehicleNumberPlateText(vehicle),currentFuel,'147')
		TriggerServerEvent('2d9a7bef-6bb2-4079-a0d6-e49807cf34eb', currentCost)
	end

	currentCost = 0.0
end)

AddEventHandler('d8353865-2e75-4065-8fe6-8e5890e60bf4', function(pumpObject, ped, vehicle)
	...
	TriggerEvent('0163347a-34b9-4cc6-a337-58fc26599cc7', pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if ESX ~= nil and PlayerData ~= nil and PlayerData.inventory ~= nil then
			for i=1, #PlayerData.inventory, 1 do
				if PlayerData.inventory[i].name == 'weapon_petrolcan' then
					remaining_fuel = PlayerData.inventory[i].count
				end
			end
		end
			DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Config.Strings.CancelFuelingJerryCan .. "\nGas can: ~g~" .. Round(remaining_fuel/ 4500 * 100, 1) .. "% | Vehicle: " .. Round(currentFuel, 1) .. "%")
		end
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				if DoesEntityExist(vehicle) and #(GetEntityCoords(ped) - vehicleCoords) < 4.5 then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								...
								if IsControlJustReleased(0, 38) then
									...
									TriggerEvent('d8353865-2e75-4065-8fe6-8e5890e60bf4', isNearPump, ped, vehicle)
									LoadAnimDict("timetable@gardener@filling_can")
								end
							else
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								TriggerServerEvent('044415d5-4418-4fcc-b008-ccb642df68a3', Config.JerryCanCost)
							...
							if Config.UseESX then
								...
								if refillCost > 0 then
									if currentCash >= refillCost then
										...
										if IsControlJustReleased(0, 38) then
											TriggerServerEvent('044415d5-4418-4fcc-b008-ccb642df68a3', refillCost)
