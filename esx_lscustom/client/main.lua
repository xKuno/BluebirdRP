...
Citizen.CreateThread(function()
	...
	Wait(2000)
	ESX.TriggerServerCallback('9db84c02-213c-4c1e-8b11-094b5501d983', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterCommand('toggleneon', function(src, args)
	local veh = GetVehiclePedIsIn(PlayerPedId(),false)
	
	if GetPedInVehicleSeat(veh,-1) == PlayerPedId() then
		if scar[tostring(veh)] == nil then
			
			scar[tostring(veh)] = false
		end
		if scar[tostring(veh)] == false then
			--DisableVehicleNeonLights(veh,true)
			SetVehicleNeonLightEnabled(veh,0,false)
			SetVehicleNeonLightEnabled(veh,1,false)
			SetVehicleNeonLightEnabled(veh,2,false)
			SetVehicleNeonLightEnabled(veh,3,false)
			scar[tostring(veh)] = true
		else
			--DisableVehicleNeonLights(veh,false)
			local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
			local myCar = ESX.Game.GetVehicleProperties(vehicle)
			ESX.TriggerServerCallback('506f4649-9007-4cf6-b1a3-5cd66ae09ec6', function(vehicles)

				if vehicles ~= nil and #vehicles ~= 0 then
					for k,v in pairs(vehicles) do
						if v == 1 then
							SetVehicleNeonLightEnabled(veh,k-1,true)
						end
					end
					scar[tostring(veh)] = false
				end
...
RegisterNetEvent('03bfc615-4fa5-44e1-af9d-90698d15bfc3')
AddEventHandler('03bfc615-4fa5-44e1-af9d-90698d15bfc3', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('fdb9c9e4-e651-44b0-a234-a21656399494', myCar)
end)

RegisterNetEvent('417d1ed3-61f0-413e-a523-b85b412e0054')
AddEventHandler('417d1ed3-61f0-413e-a523-b85b412e0054', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				local block = false
				if #elems == 2 then
				  local js = json.encode(elems)
				  
				  if string.match(js, 'Level 1') == true and string.match(js, 'Level 2') == false then
					block = true
				  end
				end
				if #elems == 1 or block == true then
					ESX.ShowNotification("Not supported on your vehicle.")
				elseif data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					ESX.ShowNotification(_U('already_own', data.current.label))
					TriggerEvent('03bfc615-4fa5-44e1-af9d-90698d15bfc3')
				else
					...
					--print ('veh price ' .. #Vehicles)
					--print(vehiclePrice)

					if isRimMod then
						price = math.floor(vehiclePrice * data.current.price / 100)
						TriggerServerEvent('08852ae6-7c05-4cfb-a12c-dfc130583e41', price)
		...
		menu.close()
		TriggerEvent('417d1ed3-61f0-413e-a523-b85b412e0054')
...
function UpdateMods(data)
	...
	if data.modType then
		...
		if props["modFrontWheels"] then
			props["modBackWheels"] = props["modBackWheels"]
		end
			...
			if v.modType then
				...
				elseif v.modType == 23 then -- WHEELS RIM & TYPE
					...
					for j = 0, modCount, 1 do
						...
						if modName and not string.match(string.lower(GetLabelText(modName)),string.lower('guns')) and not string.match(string.lower(GetLabelText(modName)),string.lower('weapon')) and not string.match(string.lower(GetLabelText(modName)),string.lower('turret')) then
							local _label = ''
				...
				elseif v.modType == 17 then -- TURBO
					...
					for j = 0, modCount, 1 do
						...
						if modName and not string.match(string.lower(GetLabelText(modName)),string.lower('guns')) and not string.match(string.lower(GetLabelText(modName)),string.lower('weapon')) and not string.match(string.lower(GetLabelText(modName)),string.lower('turret')) then
							local _label = ''
							if j == currentMods[k] then
								_label = GetLabelText(modName) .. ' - <span style="color:cornflowerblue;">A'.. _U('installed') ..'</span>'
							else
								price = math.floor(vehiclePrice * v.price / 100)
								_label = GetLabelText(modName) .. ' - <span style="color:green;">A$' .. price .. ' </span>'
							end
							table.insert(elements, {label = _label, modType = k, modNum = j})
						end
					end
...
Citizen.CreateThread(function()
	while true do
		...
		if Lisinmarker == true then
			isInLSMarker = true
		else
			isInLSMarker = false
		end
		
		if isInLSMarker == true and hasAlreadyEnteredMarker == false then
			hasAlreadyEnteredMarker = true
		end

		if  isInLSMarker == false and hasAlreadyEnteredMarker == true then
...
Citizen.CreateThread(function()
	while true do
		...
		if isInLSMarker == true and IsControlJustPressed(0, 38) and not lsMenuIsShowed then
			local playerPed = PlayerPedId()
			if IsPedInAnyVehicle(playerPed, false) then
				ESX.ShowHelpNotification('Vehicle mods are at your own risk. ~r~No refunds or exchanges~w~ given. LS Customs Management')
				if (PlayerData.job and PlayerData.job.name == 'mechanic') or not Config.IsMechanicJobOnly then
					lsMenuIsShowed = true
