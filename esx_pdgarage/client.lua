...
Citizen.CreateThread(function()
	while true do
		...
		if (ESX.PlayerData.job and (ESX.PlayerData.job.name == Config.PoliceDatabaseName or ESX.PlayerData.job.name == Config.PoliceDatabaseName2)) then
			local closer = false
			...
			for k,v in pairs(Config.HeliZones) do
				for i = 1, #v.Pos, 1 do
					local distance = Vdist2(coords, v.Pos[i].x, v.Pos[i].y, v.Pos[i].z)
					if (distance < 10.0) and insideMarker == false then
			...
			for k,v in pairs(Config.ExtraZones) do
				for i = 1, #v.Pos, 1 do
					...
					if (distance < 35.0) and insideMarker == false and pedInVeh then
					...
					if (distance < 9.5 ) and insideMarker == false and pedInVeh then
...
VehicleMenu = function(type)
	...
	function(data, menu)
		...
		TriggerEvent('854f3ac4-20f6-456f-8f0c-9d8eee5fb1b4',veh,100.0)
...
function OpenTintMenu()
	...
	for id=0,6 do
			...
			if id == tintval then
				table.insert(elements, {
					label = "Tint: "..tintlabels[id+1].." | "..('<span style="color:green;">%s</span>'):format("On"),
					value = id,
					state = not state
				})
			else
				table.insert(elements, {
					label = "Tint: "..tintlabels[id+1].." | "..('<span style="color:red;">%s</span>'):format("Off"),
					value = id,
					state = not state
				})
			end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'extra_actions', {
		...
		SetVehicleWindowTint(vehicle,tonumber(data.current.value))
		OpenTintMenu()
	end, function(data, menu)
		menu.close()
	end)
...
function OpenMainMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'color_menu', {
		...
		elseif data.current.value == 'pearl' then
			OpenMainColorMenu('pearl')
...
function OpenMyPlate()
        ESX.UI.Menu.Open(
          'dialog', GetCurrentResourceName(), 'PLATE',
          {
            title = 'New Plate (MAX 8 CHARS)'
          },
          function(data, menu)
			...
			if vehicle ~= 0 then
				...
				TriggerServerEvent('7fcbafb2-e15d-4e94-9a71-c348def151ae',"POLICE ACTION", "DRAG "  .. "CHANGE PLATE FROM: " .. oldplate .. " NEW PLATE: " .. plate)
...
function OpenColorMenu(type, value, colortype)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'extra_actions', {
		...
		elseif colortype == 'pearl' then
			SetVehicleExtraColours(vehicle, data.current.index, wheelColor)
		end
