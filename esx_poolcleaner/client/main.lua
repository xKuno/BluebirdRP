...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('84ede100-2775-4add-b133-6ef5c52ec2ef')
		StartNPCJob()
		Done = true
	end
...
AddEventHandler('b1cc42ff-26ef-447d-bf79-a55c4145fe7f', function(zone)
...
AddEventHandler('2c2b4f8c-08b0-494c-8eb4-71f23f9c2cc9', function(zone)

	if zone == 'Sale' then
		TriggerServerEvent('c51dbcbf-778a-4098-96c3-24594acb2a46')
	end
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
		...
		if PlayerData.job ~= nil then
			if PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
				...
				if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
					...
					TriggerEvent('b1cc42ff-26ef-447d-bf79-a55c4145fe7f', currentZone)
				end
				if not isInMarker and HasAlreadyEnteredMarker then
					HasAlreadyEnteredMarker = false
					TriggerEvent('2c2b4f8c-08b0-494c-8eb4-71f23f9c2cc9', LastZone)
				end
			else
				Wait(2000)
			end
...
-- Things to do after pressing E at a blip.
	Citizen.CreateThread(function()
		while true do
			Citizen.Wait(0)
			if PlayerData and PlayerData.job and PlayerData.job.name == Config.nameJob then
					...
					if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
						...
						if PlayerData.job.name == Config.nameJob then
							...
							if CurrentAction == 'Sale' then
								TriggerServerEvent('86f0bb2c-a6c2-49df-9c8f-78095fd25a10')
							end
