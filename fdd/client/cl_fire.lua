...
RegisterNetEvent(  'fdd:StartFireClient')
AddEventHandler(  'fdd:StartFireClient', function(firetable) 
    StopFireInRange(firetable[1].coords, 100.0)
    Wait(700) 
    for i=1, #firetable, 1 do
        Wait(0)
		print('fire starting in client')
...
RegisterNetEvent(  'fdd:GetCoords')
AddEventHandler(  'fdd:GetCoords', function(position, flames, radius)  
	print('request for fire')
	print('flames ' .. flames)
	print('radius ' .. radius)
    FireRadius = radius
    local FireSetup = {}
    for i=1, flames, 1 do
        ...
        if groundz > 1.0 then
			--print('fire start up using .z')
			--print(json.encode(position))
            table.insert(FireSetup, {
                coords = vec3(FlamePos.x, FlamePos.y, groundz + 0.25),
            })
		else
			--print('fire start up ON OUR OWN')
			--print(json.encode(position))
		      table.insert(FireSetup, {
                coords = vec3(position.x, position.y, position.z + 0.25),
            })
        end
    end
	
	TriggerServerEvent(  'fdd:ReturnCoords', FireSetup)

end)

-- Kill all flames and cleanup the fire --
RegisterNetEvent(  'fdd:StopFireSync')
AddEventHandler(  'fdd:StopFireSync', function(position)  
    Citizen.Wait(5000)
    FireDone(position)
    FireActive = false
end)
...
RegisterNetEvent(  'fdd:FireTimeout')
AddEventHandler(  'fdd:FireTimeout', function()  
    FireActive = false
    FireTimeout()
end)

-- Sync flames between clients --
RegisterNetEvent(  'fdd:SyncFlames')
AddEventHandler(  'fdd:SyncFlames', function(position)  
...
RegisterNetEvent('880c0a93-b305-46b3-b84c-a120fee9e841')
AddEventHandler('880c0a93-b305-46b3-b84c-a120fee9e841', function()
    if not AlreadySpawned then
        AlreadySpawned = true
        TriggerServerEvent(  'fdd:PlayerJoined')
    end
end)
            ...
            while size > 0 do
                ...
                if size > 0.01 then
                    ...
                    if FiresInArea <= 3 then
                        Wait(100)
                        TriggerServerEvent(  'fdd:StopFire', position)
                    end
                    break
                end
            end
