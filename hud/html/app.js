...
window.onload = function () {
		...
		var eventCallback = {
			...
			setVoiceDistance: function(data) {
				...
				if (data.value == 'phone')
				{
					var micicon = document.querySelector('#mici');
					var miciconIdWithClasses = micicon.classList;
					miciconIdWithClasses.remove('fas','fa-microphone-alt','fa-bullhorn','fa-mobile-alt','fa-car','fa-comment');	
					miciconIdWithClasses.add('fas');
					miciconIdWithClasses.add('fa-mobile-alt');
					
				} 
				else if (data.value == 'pa')
				{
					var micicon = document.querySelector('#mici');
					var miciconIdWithClasses = micicon.classList;
					miciconIdWithClasses.remove('fas','fa-microphone-alt','fa-bullhorn','fa-mobile-alt','fa-car','fa-comment');	
					miciconIdWithClasses.add('fas');
					miciconIdWithClasses.add('fa-bullhorn');
				}
				else if (data.value == 'car')
				{
					...
					miciconIdWithClasses.remove('fas','fa-microphone-alt','fa-bullhorn','fa-mobile-alt','fa-car','fa-comment');	
					miciconIdWithClasses.add('fas');
					miciconIdWithClasses.add('fa-car');
				}
				else if (data.value == 'conversation')
				{
					var micicon = document.querySelector('#mici');
					var miciconIdWithClasses = micicon.classList;
					miciconIdWithClasses.remove('fas','fa-microphone-alt','fa-bullhorn','fa-mobile-alt','fa-car','fa-comment');	
					miciconIdWithClasses.add('fas');
					miciconIdWithClasses.add('fa-comment');
					
				} 
				
				else if (data.value == 'whisper')
				{
					var micicon = document.querySelector('#mici');
					var miciconIdWithClasses = micicon.classList;
					miciconIdWithClasses.remove('fas','fa-microphone-alt','fa-bullhorn','fa-mobile-alt','fa-car','fa-comment')	;
					miciconIdWithClasses.add('fas');
					miciconIdWithClasses.add('fa-microphone-alt');
				...
				voiceIdWithClasses.remove('whisper', 'normal', 'shout','phone','pa','car','conversation');
			...
			updateVehicle: function(data) {
				...
				var vehicleOil = document.querySelector('#oil');
				var vehicleCruiser = document.querySelector('#vehicle-speed strong');
				var vehicleODO = document.querySelector('#odo')
				...
				if (data.status == true) {
					if (vehicleInfo.classList.contains('inactive')) {
						...
						vehicleOil.style.display = 'none';
						vehicleODO.style.display = 'none';
						...
						if (data.type)
						{
							if (vehiclesCars.indexOf(data.type) > -1) {
								...
								vehicleOil.style.display = 'block';
								vehicleODO.style.display = 'block';
					...
					if (data.fuel != null)
					{
						...
						if (data.oil){
							vehicleOil.querySelector('span').style.height = data.oil + '%';
						if (data.oil <= 10) {
							if (vehicleOil.classList.contains('dying') == false) 
								{ vehicleOil.classList.add('dying');	}
						}
						else { 
							vehicleOil.classList.remove('dying'); 
						}}
					
					
					if (data.odo != null)
					{
						saferInnerHTML(document.querySelector('#odo2'), data.odo);
					}
			...
			toggleUi: function(data) {
				...
				if (data.value == true) {
					...
					document.querySelector('.info.time-and-place').style.display = 'block'; 
					...
					document.querySelector('.info.time-and-place').style.display = 'none'; 
