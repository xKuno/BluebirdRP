...
RegisterCommand('removewheelchair', function()
	local wheelchair = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 10.0, `prop_wheelchair_01`)
...
Citizen.CreateThread(function()
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			local v3ped = vector3(pedCoords["x"],pedCoords["y"],pedCoords["z"])
			local v3sit	= vector3(sitCoords["x"],sitCoords["y"],sitCoords["z"])
			local v3pickup = vector3(pickupCoords["x"],pickupCoords["y"],pickupCoords["z"])
			
			if #(v3ped - v3sit) <= 1.0 then
