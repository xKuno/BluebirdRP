...
function radiopttpress()
	if Radio.On then
		isBroadcasting = true
	end
end

function radiopttdepress()
	if Radio.On then
...
exports("pttpress",radiopttpress)
exports("pttdepress",radiopttdepress)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if Radio.Open then
			...
			AddTextComponentSubstringPlayerName(exports["mumble-voip"]:getCurrentRadioChannelName(radioConfig.Frequency.Current))
			...
			if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
				if (Radio.On == false and exports["esx_policejob"]:hasanyradio() == true) or Radio.On == true then
...
RegisterNetEvent("RadioToggleoff")
AddEventHandler("RadioToggleoff", function()
...
exports('ClientRadioOff', function(args)
	Radio:Remove()
	exports["mumble-voip"]:SetMumbleProperty("radioEnabled", false)
	Radio:Toggle(false)
	Radio.On = false
