...
local toggle_helicam = 38--51 -- control id of the button by which to toggle the helicam mode. Default: INPUT_CONTEXT (E)
...
local toggle_spotlight = 20 -- control id to toggle the front spotlight Default: INPUT_PhoneCameraGrid (G)
local toggle_lock_on = 22 -- control id to lock onto a vehicle with the camera. Default is INPUT_SPRINT (spacebar)

-- Script starts here
local helicam = false

local helicopterm = {`polmav`,`swift2`,`aw139`,`polaw139`,`supervolito`} 
	...
	while true do
		...
		if IsPlayerInPolmav() then
			...
			if IsHeliHighEnough(heli) then
				...
				if GetLastInputMethod(0) and IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
					
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					helicam = true
				end
				
				if GetLastInputMethod(0) and IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
			...
			if GetLastInputMethod(0) and IsControlJustPressed(0, toggle_spotlight)  and GetPedInVehicleSeat(heli, -1) == lPed then
				spotlight_state = not spotlight_state
				TriggerServerEvent('11d4b973-ebfb-44c6-81ed-88d38a08f399', spotlight_state)
				PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
			end
			...
			if helicam then
				...
				while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
					...
					if GetLastInputMethod(0) and IsControlJustPressed(0, toggle_vision) then
						PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
						ChangeVision()
					end
					...
					if locked_on_vehicle then
						if DoesEntityExist(locked_on_vehicle) then
							...
							if GetLastInputMethod(0) and IsControlJustPressed(0, toggle_lock_on) then
								PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
...
RegisterNetEvent('e4f9a34d-50a9-4ef1-857e-d419d451ca67')
AddEventHandler('e4f9a34d-50a9-4ef1-857e-d419d451ca67', function(serverID, state)
...
function IsPlayerInPolmav()
	...
	for i = 1 , #helicopterm , 1  do
		if helicopterm[i] == model then
...
function HandleZoom(cam)
	if GetLastInputMethod(0) and IsControlJustPressed(0,241) then -- Scrollup
		fov = math.max(fov - zoomspeed, fov_min)
	end
	if GetLastInputMethod(0) and IsControlJustPressed(0,242) then
...
-- if GetLastInputMethod(0) and IsControlJustPressed(0, toggle_spotlight) then
	-- PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
	-- spotlight_state = not spotlight_state
-- end
