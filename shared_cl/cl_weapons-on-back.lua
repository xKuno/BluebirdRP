...
local SETTINGS = {
    ...
    compatable_weapon_hashes = {
        -- melee:
        ["prop_golf_iron_01"] = 2132975508,--1141786504, -- positioning still needs work
        ["w_me_bat"] = -1786099057,
  ...
  while true do
	  ...
	  if me ~= storedme then
	  	for name, attached_object in pairs(attached_weapons) do
			...
			DeleteEntity(attached_object.handle)
      ...
      for wep_name, wep_hash in pairs(SETTINGS.compatable_weapon_hashes) do
          if HasPedGotWeapon(me, wep_hash, false) and selectedweapon ~= wep_hash then
      ...
      for name, attached_object in pairs(attached_weapons) do
          -- equipped? delete it from back:
          if selectedweapon ==  attached_object.hash or not HasPedGotWeapon(me, attached_object.hash, false) then -- equipped or not in weapon wheel
...
function AttachWeapon(attachModel,modelHash,boneNumber,x,y,z,xR,yR,zR, isMelee)
  ...
  attached_weapons[attachModel] = {
	...
	local attempts = 10
	while NetworkGetNetworkIdFromEntity(attached_weapons[attachModel].handle) == 0 and attempts > 0 do
		attempts = attempts - 1
		ObjToNet(attached_weapons[attachModel].handle)
		Wait(10)
	end
