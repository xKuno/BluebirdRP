...
function takeHostage()
	...
	for i=1, #hostageAllowedWeapons do
		if HasPedGotWeapon(GetPlayerPed(-1), hostageAllowedWeapons[i], false) and GetSelectedPedWeapon(GetPlayerPed(-1)) == hostageAllowedWeapons[i] then
	...
	if not canTakeHostage then 
		Wait(1000)
		drawNativeNotification("You need a handgun or knife to take a hostage.")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		local loc1 = GetEntityCoords(GetPlayerPed(-1))
		local loc2 = GetEntityCoords(target)
		local distance = #(loc1 - loc2)
		
		if closestPlayer ~= -1 and closestPlayer ~= nil and distance < 1.3 and GetVehiclePedIsIn(target,false) == 0 then
			SetCurrentPedWeapon(GetPlayerPed(-1), foundWeapon, true)
			holdingHostageInProgress = true
			holdingHostage = true 
			TriggerServerEvent('920b987c-22a3-48ac-adfd-92d70c4b176f', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
		else
...
RegisterNetEvent('381e5f8d-6a23-4bb3-8c45-176f8ae8dc3e')
AddEventHandler('381e5f8d-6a23-4bb3-8c45-176f8ae8dc3e', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
...
RegisterNetEvent('b3fb2e0f-b750-40b4-89f9-e56d5769c96e')
AddEventHandler('b3fb2e0f-b750-40b4-89f9-e56d5769c96e', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	ClearPedSecondaryTask(GetPlayerPed(-1))
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
RegisterNetEvent('4ce6b0d5-7b51-4991-abe0-4654e4edf300')
AddEventHandler('4ce6b0d5-7b51-4991-abe0-4654e4edf300', function()
	holdingHostageInProgress = false
	beingHeldHostage = false 
	...
	while true do 
		if holdingHostage then
			if IsEntityDead(GetPlayerPed(-1)) then	
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent('855f73d0-cba3-4111-aa7f-7c13a829793d',target)
				Wait(100)
				releaseHostage()
			end 
		...
		if beingHeldHostage == false and  holdingHostage == false then
			Wait(1500)
		end
		Wait(0)
	end
