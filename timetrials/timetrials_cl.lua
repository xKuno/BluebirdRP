-- Local parameters
local START_PROMPT_DISTANCE = 6.0              -- distance to prompt to start race
local DRAW_TEXT_DISTANCE = 30.0                -- distance to start rendering the race name text
local DRAW_SCORES_DISTANCE = 15.0               -- Distance to start rendering the race scores
    ...
    while raceState.index == 0 do
        ...
        for index, race in pairs(races) do
            if race.isEnabled then
                -- Draw map marker
				local racev3 = vector3( race.start.x, race.start.y, race.start.z - 1)
                
                local entitycoords = GetEntityCoords(player)
				
				local distancec = #(racev3 - entitycoords)
                -- Check distance from map marker and draw text if close enough
                if distancec < DRAW_TEXT_DISTANCE then
					closest = distancec
                    -- Draw race name
					...
					if distancec < DRAW_SCORES_DISTANCE then
					...
					if distancec  < START_PROMPT_DISTANCE then
						...
						if (IsControlJustReleased(1, 51)) then
							...
							TriggerEvent('75203db2-95d9-40ff-ba8a-b03e1f2f4e84')
							break
						end
					end
               end 
...
RegisterNetEvent('9114f56b-2348-4c24-aeb2-5c0ae9f8b09d')
AddEventHandler('9114f56b-2348-4c24-aeb2-5c0ae9f8b09d', function(scores)
    -- Save scores to state
    raceState.scores = scores
end)

-- Countdown race start with controls disabled
RegisterNetEvent('75203db2-95d9-40ff-ba8a-b03e1f2f4e84')
AddEventHandler('75203db2-95d9-40ff-ba8a-b03e1f2f4e84', function()
    -- Get race from index
    ...
    TriggerEvent('2c4f07a6-bf05-4b02-ba86-fd4472391cbd',"~r~Remember:~w~Racing on public roads is ~o~illegal~w~.")
    Citizen.CreateThread(function()
        -- Countdown timer
        local time = 0
        ...
        TriggerEvent('2c4f07a6-bf05-4b02-ba86-fd4472391cbd',"~r~Remember:~w~Racing on public roads is ~o~illegal~w~.")
        -- Start race
        TriggerEvent('e219e01b-e4aa-403c-919d-61cf28d32daf')
    end)
end)

-- Main race function
RegisterNetEvent('e219e01b-e4aa-403c-919d-61cf28d32daf')
AddEventHandler('e219e01b-e4aa-403c-919d-61cf28d32daf', function()
    -- Get race from index
    local race = races[raceState.index]
    TriggerEvent('2c4f07a6-bf05-4b02-ba86-fd4472391cbd',"~r~Remember:~w~Racing on public roads is ~o~illegal~w~.")
    -- Start a new timer
        ...
        while raceState.index ~= 0 do 
            ...
            if GetDistanceBetweenCoords(race.checkpoints[raceState.cP].x,  race.checkpoints[raceState.cP].y,  race.checkpoints[raceState.cP].z, GetEntityCoords(GetPlayerPed(-1))) < race.checkpointRadius then
                ...
                if raceState.cP == #(race.checkpoints) then
                    ...
                    TriggerServerEvent('00f03d22-fd7c-4830-9a22-82e387a03bf1', GetPlayerName(PlayerId()), message, race.title, score)
...
Citizen.CreateThread(function()
    ...
    for _, race in pairs(races) do
        if race.isEnabled and race.hideBlip == null then
