...
function menuToggle(bool,send=false) {
	...
	if(send){
		$.post('http://tuner/togglemenu', JSON.stringify({state:false}));
	}
}
...
$(function(){
	...
	$("#boost").slider({min:0.05,value:0.25,step:0.01,max:0.90,change:sliderUpdated});
	$("#fuelmix").slider({min:1,value:1.3,step:0.01,max:2,change:sliderUpdated});
	$("#gearchange").slider({min:1,value:12,max:60,step:5,change:sliderUpdated});
	$("#braking").slider({min:0.0,value:0.5,max:1.2,step:0.05,change:sliderUpdated});
	...
	$("#savebtn").click(function(){
		initiateTyepwriter();
		$.post('http://tuner/save', JSON.stringify(getSliderValues()));
	});
